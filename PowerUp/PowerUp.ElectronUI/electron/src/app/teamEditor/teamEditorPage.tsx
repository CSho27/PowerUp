...
import {
  LoadTeamEditorApiClient,
  LoadTeamEditorResponse,
} from './loadTeamEditorApiClient';
import {
  PlayerRoleRequest,
  SaveTeamApiClient,
  SaveTeamRequest,
} from './saveTeamApiClient';
import {
  getDetailsReducer,
...
export function TeamEditorPage() {
  return (
    <PageLoader
      ...
      renderPage={p => <InternalTeamEditorPage {...p} />}
    />
  );
}

function InternalTeamEditorPage(props: TeamEditorPageProps) {
  const appContext = useAppContext();
  const { teamId, editorResponse } = props;
  ...
  const pitchers = mlbPlayers.filter(
    p => p.playerDetails.position === 'Pitcher'
  );
  ...
  const editorDisabled: DisabledCriterion = {
    isDisabled: !canEdit,
    tooltipIfDisabled: 'Teams of this type cannot be edited',
  };
  const noUnsavedChangesDisabled: DisabledCriterion = {
    isDisabled: !hasUnsavedChanges,
    tooltipIfDisabled: 'No changes',
  };
  const actionsDisabled: DisabledCriterion[] = [
    editorDisabled,
    noUnsavedChangesDisabled,
  ];
  ...
  const header = (
    <>
      ...
      <TabButtonNav
        ...
        onChange={t =>
          update({ type: 'updateSelectedTab', selectedTab: t as TeamEditorTab })
        }
      />
    </>
  );
  ...
  return (
    <PowerUpLayout headerText='Edit Team'>
        ...
        <EditorContainer>
          ...
          {state.selectedTab === 'Pitcher Roles' && (
            <PitcherRolesEditor
              ...
              disabled={[editorDisabled]}
              updateRole={(id, role, index) =>
                updateCurrentDetails({
                  type: 'updatePitcherRole',
                  playerId: id,
                  role: role,
                  orderInRole: index,
                })
          ...
          {state.selectedTab === 'No DH Lineup' && (
            <LineupEditor
              ...
              useDh={false}
              disabled={[editorDisabled]}
              updateLineupOrder={(id, currentOrder, newOrder) =>
                updateCurrentDetails({
                  type: 'reorderNoDHLineup',
                  playerIdentifier: id,
                  currentOrderInLineup: currentOrder,
                  newOrderInLineup: newOrder,
                })
              }
              swapPositions={(p1, p2) =>
                updateCurrentDetails({
                  type: 'swapPositionInNoDHLineup',
                  position1: p1,
                  position2: p2,
                })
              }
  ...
  function toHitterDetails(
    ...
    return {
      orderInLineup: useDh ? player.orderInDHLineup : player.orderInNoDHLineup,
      position: useDh ? player.positionInDHLineup : player.positionInNoDHLineup,
      details: {
        sourceType: playerDetails.sourceType,
        playerId: playerDetails.playerId,
        savedName: playerDetails.savedName,
  ...
  async function saveTeam(persist: boolean) {
    const response = await apiClientRef.current.execute(
      buildSaveRequest(currentDetails, persist)
    );
    if (persist && response.success) update({ type: 'updateFromSave' });
  }

  function buildSaveRequest(
    teamDetails: TeamEditorDetails,
    persist: boolean
  ): SaveTeamRequest {
    return {
      teamId: teamId,
  ...
  async function undoChanges() {
    ...
    apiClientRef.current.execute(
      buildSaveRequest(state.lastSavedDetails, false)
    );
  }
}
...
async function loadTeamEditorPageProps(
  appContext: AppContext,
  params: RouteParams
) {
  const teamId = Number.parseInt(params.teamId ?? '');
  const tempTeamId = params.tempTeamId
...
export const cleanupTeamEditorPage: PageCleanupFunction = async (
  appContext: AppContext,
