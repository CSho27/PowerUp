...
function InternalRosterEditorPage(props: RosterEditorPageProps) {
  ...
  const rosterNameApiClientRef = useRef(
    new EditRosterNameApiClient(appContext.commandFetcher)
  );
  const replaceFreeAgentApiClientRef = useRef(
    new ReplaceFreeAgentApiClient(appContext.commandFetcher)
  );

  const divisionOptions: KeyedCode[] = [
    ...props.divisionOptions,
    { key: 'FreeAgents', name: 'Free Agents' },
  ];
  ...
  const disableRosterEdit: DisabledCriteria = [
    {
      isDisabled: !rosterDetails.canEdit,
      tooltipIfDisabled: 'Roster of this type cannot be edited',
    },
  ];
  ...
  const header = (
    <>
      ...
      >
        ...
        >
          {!isEditingRosterName && (
            <>
              <RosterHeader>
                {rosterName} - {toIdentifier('Roster', rosterId)}
              </RosterHeader>
              <SourceTypeStamp
                theme='Dark'
                size='Medium'
        ...
        <div style={{ display: 'flex', gap: '16px', alignItems: 'center' }}>
          <Button
            ...
            icon='list-ol'
            onClick={() =>
              appContext.setPage({ page: 'DraftPage', rosterId: rosterId })
            }
          />
          <Button
            size='Medium'
      ...
      <TabButtonNav
        ...
        onChange={t =>
          handleDivisionChange(divisionOptions.find(o => o.name === t)!)
        }
      />
    </>
  );
  ...
  return (
    <PowerUpLayout headerText='Edit Roster'>
      ...
      >
        <TeamsContainer>
          {teams
            .filter(t => t.division === selectedDivision.key)
            .map(toTeamGrid)}
  ...
  function toTeamGrid(team: TeamDetails) {
    return (
      <TeamWrapper key={`${team.teamId} - ${team.powerProsTeam}`}>
        <TeamGrid
          appContext={appContext}
          rosterId={rosterId}
          disableRosterEdit={disableRosterEdit}
          team={team}
        />
      </TeamWrapper>
    );
  ...
  async function handleRosterNameToggle() {
    ...
    const response = await rosterNameApiClientRef.current.execute({
      rosterId: rosterId,
      rosterName: rosterName,
    });
    if (response.success) setIsEditingRosterName(false);
  }
