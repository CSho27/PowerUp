...
import {
  LoadPlayerEditorApiClient,
  PlayerEditorResponse,
} from './loadPlayerEditorApiClient';
import { PitcherAbilitiesEditor } from './pitcherAbilitiesEditor';
import {
  buildSavePlayerRequestFromState,
  getAppearanceReducer,
  getDetailsReducer,
  getHitterAbilitiesReducer,
  getInitialStateFromResponse,
  getPersonalDetailsReducer,
  getPitcherAbilitiesReducer,
  getPositionCapabilityDetailsReducer,
  getSepcialAbilitiesReducer,
  PlayerEditorReducer,
  PlayerEditorTab,
  playerEditorTabOptions,
  PlayerPersonalDetailsContext,
} from './playerEditorState';
...
export function PlayerEditorPage() {
  return (
    <PageLoader
      ...
      renderPage={p => <InternalPlayerEditorPage {...p} />}
    />
  );
}

function InternalPlayerEditorPage(props: PlayerEditorPageProps) {
  const appContext = useAppContext();
  const { playerId, editorResponse } = props;
  const { sourceType, canEdit, options } = editorResponse;

  const apiClientRef = React.useRef(
    new SavePlayerApiClient(appContext.commandFetcher)
  );

  const reducerContext: PlayerPersonalDetailsContext = {
    swingManRole: options.personalDetailsOptions.pitcherTypes.find(
      t => t.key === 'SwingMan'
    ) as KeyedCode,
    starterRole: options.personalDetailsOptions.pitcherTypes.find(
      t => t.key === 'Starter'
    ) as KeyedCode,
  };

  const [state, update] = useReducerWithContext(
    PlayerEditorReducer,
    getInitialStateFromResponse(editorResponse),
    reducerContext
  );
  const [currentDetails, updateCurrentDetails] = getDetailsReducer(
    state,
    update
  );
  const [personalDetails, updatePersonalDetails] = getPersonalDetailsReducer(
    currentDetails,
    updateCurrentDetails
  );
  const [apperance, updateAppearance] = getAppearanceReducer(
    currentDetails,
    updateCurrentDetails
  );
  const [positionCapabilityDetails, updatePositionCapabilities] =
    getPositionCapabilityDetailsReducer(currentDetails, updateCurrentDetails);
  const [hitterAbilities, updateHitterAbilities] = getHitterAbilitiesReducer(
    currentDetails,
    updateCurrentDetails
  );
  const [pitcherAbilities, updatePitcherAbilities] = getPitcherAbilitiesReducer(
    currentDetails,
    updateCurrentDetails
  );
  const [specialAbilities, updateSpecialAbilities] = getSepcialAbilitiesReducer(
    currentDetails,
    updateCurrentDetails
  );
  ...
  const positionType = getPositionType(
    currentDetails.personalDetails.position.key as Position
  );

  const hasUnsavedChanges = !deepEquals(
  ...
  const header = (
    <>
      ...
      <PlayerHeaderContainer>
        ...
        <div>
          <PositionBubble positionType={positionType} size='Large'>
            {currentDetails.personalDetails.position.name}
          </PositionBubble>
        </div>
        <div>
          <OutlineHeader
            fontSize={FONT_SIZES._40}
            strokeWeight={2}
            textColor={COLORS.primaryBlue.regular_45}
          ...
          <div>
            <PlayerHeaderActionButtons>
              <Link
                ...
                url={editorResponse.baseballReferenceUrl}
              >
                BBRef
              </Link>
              <Button
            ...
            {!!state.dateLastSaved && (
              <span>
                Last Save: {toShortDateTimeString(state.dateLastSaved, true)}
              </span>
            )}
          </div>
        </PlayerHeaderActions>
      ...
      <TabButtonNav
        ...
        onChange={t =>
          update({
            ...
            selectedTab: t as PlayerEditorTab,
          })
        }
      />
    </>
  ...
  return (
    <PowerUpLayout headerText='Edit Player'>
        ...
        <EditorContainer>
          {state.selectedTab === 'Personal' && (
            <PlayerPersonalDetailsEditor
              ...
              initiallyHadSpecialSavedName={
                editorResponse.personalDetails.isSpecialSavedName
              }
          ...
          {state.selectedTab === 'Hitter' && (
            <HitterAbilitiesEditor
              battingSide={
                currentDetails.personalDetails.battingSide.key as BattingSide
              }
  ...
  async function savePlayer() {
    const response = await apiClientRef.current.execute(
      buildSavePlayerRequestFromState(currentDetails, playerId)
    );
    if (response.success) update({ type: 'updateFromSave' });
  }
}
...
async function loadPlayerEditorPageProps(
  appContext: AppContext,
  params: RouteParams
) {
  const playerId = Number.parseInt(params.playerId ?? '');
  const apiClient = new LoadPlayerEditorApiClient(appContext.commandFetcher);
  const response = await apiClient.execute({ playerId: playerId });
  return {
