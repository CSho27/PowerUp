...
export function AppBehavior({
  ...
  const [config, setConfig] = useState<AppContextConfig | null>(null);
  ...
  useEffect(() => {
    new InitializeBaseRosterApiClient(appContext.commandFetcher).execute();
  }, []);

  return (
    <AppContextProvider appContext={appContext} setConfig={setConfig}>
      {children}
  ...
  async function popBreadcrumb(breadcrumbId: number) {
    ...
    update({
      type: 'updatePageFromBreadcrumb',
      breadcrumbId: breadcrumbId,
      pageLoadDef: pageLoadDef,
    });
  }

  function openModalAsync<T>(
    renderModal: AsyncRenderModalCallback<T>
  ): Promise<T> {
  ...
  function openModal(renderModal: RenderModalCallback) {
    ...
    update({
      type: 'openModal',
      modal: { key: key, modal: renderModal(closeDialog) },
    });
  }
  ...
  async function performWithSpinner<T>(action: () => Promise<T>): Promise<T> {
    ...
    var returnValue = await action().catch(e => {
      alert(
        'Unexpected Error: If this error persists please create a GitHub issue and contact the creator.'
      );
