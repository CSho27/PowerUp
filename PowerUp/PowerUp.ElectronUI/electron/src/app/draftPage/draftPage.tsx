...
import {
  DraftStateReducer,
  getInitialState,
  getNextPickingPlayherIndex as getNextPickingPlayerIndex,
  getDraftingIndex,
  getRound,
} from './draftState';
...
import {
  fromOptions,
  toOptions,
} from '../../components/SelectField/selectFieldHelpers';
...
export function DraftPage() {
  return (
    <PageLoader
      ...
      renderPage={p => <InternalDraftPage {...p} />}
    />
  );
}

function InternalDraftPage({ rosterId, existingTeams }: DraftPageProps) {
  const appContext = useAppContext();
  const [state, update] = useReducer(DraftStateReducer, getInitialState(2));
  const draftPoolApiClient = useMemo(
    () => new DraftPoolApiClient(appContext.commandFetcher),
    [appContext.commandFetcher]
  );
  const replacerApiClient = useMemo(
    () => new ReplaceWithDraftedTeamsApiClient(appContext.commandFetcher),
    [appContext.commandFetcher]
  );
  ...
  const leftTeamIndex =
    state.numberOfTeams === 2 && draftingIndex === 0
      ? draftingIndex
      : nextDraftingIndex;
  const rightTeamIndex =
    state.numberOfTeams === 2 && draftingIndex === 0
      ? nextDraftingIndex
      : draftingIndex;
  ...
  const leftTeamPlayers = leftTeam.selections.map(
    s => state.draftPool.find(p => p.playerId === s)!
  );
  const rightTeamPlayers = rightTeam.selections.map(
    s => state.draftPool.find(p => p.playerId === s)!
  );
  const undraftedPlayers = state.draftPool
    .sort(SortHelpers.numericallyByDesc(p => p.overall))
    .filter(p => !allSelections.some(s => s === p.playerId));

  return (
    <PowerUpLayout headerText='Draft Teams'>
      <ContentWithHangingHeader
        header={<Breadcrumbs appContext={appContext} />}
        headerHeight='48px'
      >
        <Wrapper>
            ...
            <PlayerGrid>
              <TeamField>
                ...
                <SelectField
                  ...
                  onChange={value =>
                    update({
                      type: 'updateReplaceTeam',
                      teamIndex: leftTeamIndex,
                      team: fromOptions(existingTeams, value),
                    })
                  }
                >
            ...
            >
              ...
              {!state.isGenerating && allSelections.length === 0 && (
                <Button onClick={generateDraftPool} size='Small' variant='Fill'>
                  {state.draftPool.length > 0 ? 'Re-generate' : 'Generate'}{' '}
                  Draft Pool
                </Button>
              )}
              ...
              {state.isGenerating && (
                ...
                >
                  <Spinner />
                  <div>
                    Generating draft pool. This should take about{' '}
                    {generationEstimate} minutes.
                  </div>
                </div>
              )}
            </div>
            <PlayerGrid>{undraftedPlayers.map(toUndradftedPlayer)}</PlayerGrid>
          </DraftPoolContainer>
  ...
  async function handleSelection(playerId: number) {
    const isFinalSelection =
      round === 25 && draftingIndex === state.teams.length - 1;

    update({ type: 'makeSelection', playerId: playerId });

    if (isFinalSelection) confirmAndComplete();
  }

  async function handleStartOver() {
    const shouldStartOver = await appContext.openModalAsync(closeAndResolve => (
      <ConfirmationModal
    ...
    if (shouldStartOver) update({ type: 'resetPicks' });
  }

  async function confirmAndComplete() {
    const shouldComplete = await appContext.openModalAsync(closeAndResolve => (
      <ConfirmationModal
    ...
    if (shouldComplete) {
      ...
      appContext.popBreadcrumb(
        appContext.breadcrumbs[appContext.breadcrumbs.length - 2].id
      );
    } else {
...
async function loadDraftPageProps(appContext: AppContext, params: RouteParams) {
  ...
  const teamOptions = response.rosterDetails.teams.map(t => ({
    key: t.powerProsTeam,
    name:
      t.name === t.powerProsName ? t.name : `${t.name} (${t.powerProsName})`,
  }));
...
const TeamContainer = styled.div<{ isDrafting: boolean }>`
  ...
  min-width: 0;
  background-color: ${p =>
    p.isDrafting ? COLORS.primaryBlue.lighter_69_t40 : undefined};
  border: 3px solid
    ${p => (p.isDrafting ? COLORS.primaryBlue.regular_45 : 'transparent')};
  padding: 0 8px;
  height: 100%;
  overflow: auto;
`;
